...
class SAdminFunct{
	...
	public function endFunctionFirst() {
		$completeGrade = $this->checkIfAllComplete();
		$completeGradeSubject = $this->checkAllStudentHasGrade();
		if ($completeGrade === true && $completeGradeSubject === true) {
			$this->getAllStudents();
			...
			$query9 = $this->conn->query("DELETE FROM admann");
			$query10 = $this->conn->query("DELETE FROM facann");
			$query11 = $this->conn->query("UPDATE system_settings SET sy_status ='Ended' where sy_status ='Current'") or die("failed8");
			if($query1){
			...
			$this->alert("Error!", "Some of the students doesn't have a complete grade yet", "error", "superadmin-system-settings");
		}
	}
